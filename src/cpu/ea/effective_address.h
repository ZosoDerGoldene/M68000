...
namespace cpu {
    ...
    namespace ea {
        ...
        namespace {
            ...
            inline address_t
            handle_full_extension_word(const address_t base_address, const execution_context &context, const word_t extension_word) {
                cpu& c = context._cpu;
                memory::memory& mem = context._memory;
                ...
                switch (bd_size) {
                    case 0b000000:
                        //throw new std::runtime_error("reserved base_displacement_size");
                            break;
                    case 0b100000:
                        address += get_displacement<word_t>(c);
                    break;
                    case 0b110000: {
                        address += get_displacement<address_t>(c);
                    }
                    break;
                    default:
                    case 0b010000:
                ...
                switch (index_indirect_select) {
                    case 0b000:
                        // no memory indirect action
                            if (index_suppress) {
                                ...
                                address += calc_index<false>(c, extension_word);
                            }
                    break;
                    case 0b001:
                    ...
                    case 0b010:
                        // word outer displacement
                            if (index_suppress) {
                                ...
                                address = mem.read<address_t>(address);
                                address += sign_extend<opcode_t>(c.fetch_extension_word());
                            } else {
                                address += calc_index<false>(c, extension_word);
                                address = mem.read<address_t>(address);
                                address += sign_extend<opcode_t>(c.fetch_extension_word());
                    ...
                    case 0b011: {
                        ...
                        long_t outer_displacement = get_displacement<long_t>(c);
                    ...
                    case 0b111:
                        if (index_suppress) {
                            ...
                            address = mem.read<address_t>(address);
                            address += calc_index<false>(c, extension_word);
                            address += get_displacement<long_t>(c);;
                        }
                    break;
                    default:
            ...
            inline address_t handle_brief_extension_word(
                address_t base_address, cpu& c, word_t extension_word) {
                slong_t displacement = sign_extend<byte_t>(static_cast<byte_t>(extension_word));
                address_t index = calc_index<brief_extension_word>(c, extension_word);
            ...
            // but the aforementioned processers would not only not recognize a full extension word, they're also
            // going to choke on the "scale" part in the brief extension word.
            template<bool brief_extension_word>
            inline address_t calc_extension_word_address(address_t base_address, execution_context& context) {
                word_t extension_word = context._cpu.fetch_extension_word();
                if constexpr (brief_extension_word) {
                    return handle_brief_extension_word<true>(base_address, context._cpu, extension_word);
                } else {
                    if (extension_word & 0b100000000) {
                        //full extension word
                        return handle_full_extension_word(base_address, context, extension_word);
                    } else {
                        // brief extension word
                        return handle_brief_extension_word<false>(base_address, context._cpu, extension_word);
                    }
                }
            }
        }
        ...
        struct dn : public register_based {
            ...
            template<typename T>
            inline T read(execution_context& context) const { return context._cpu.d(_register). template read<T>();}

            template<typename T>
            inline void write(execution_context& context, T value) { context._cpu.d(_register) = value; }
        };
        ...
        struct an : public register_based {
            ...
            template<typename T>
            inline T read(execution_context& context) { return context._cpu.a(_register). template read<T>();}

            template<typename T>
            inline void write(execution_context& context, T value) { context._cpu.a(_register) = value; }
        };
        ...
        struct an_ind : public register_based {
            ...
            template<typename T>
            inline T read(execution_context& context) {
                address_t address = context._cpu.a(_register). template read<address_t>();
                return context._memory.read<T>(address);
            }

            template<typename T>
            inline void write(execution_context& context, T value) {
                address_t address = context._cpu.a(_register). template read<address_t>();
                return context._memory.write(address, value);
            }
        };
        ...
        struct an_ind_post_incr : public register_based {
            ...
            explicit an_ind_post_incr(opcode_t reg) noexcept : register_based(reg) {}

            template<typename T>
            inline T read(execution_context& context) {
                address_t address = calc_address<T>(context);
                return context._memory.read<T>(address);
            }
        ...
        private:
            template<typename T>
            inline address_t calc_address(execution_context& context) {
                address_t address = context._cpu.a(_register). template read<address_t>();
                if constexpr(is_byte<T>) {
                    if (_register == 7) {
                        context._cpu.a(_register) = address + (offset<T>() << 1);
                    } else {
                        context._cpu.a(_register) = address + offset<T>();
                    }
                } else {
                    context._cpu.a(_register) = address + offset<T>();
                }
        ...
        private:
            template<typename T>
            inline address_t calc_address(execution_context& context) {
                address_t address = context._cpu.a(_register).template
                    read<address_t>();
                if constexpr(is_byte<T>) {
                    if (_register == 7) {
                        address -= offset<T>() * 2;
                ...
                context._cpu.a(_register) = address;
                return address;
            }
        ...
        struct an_ind_disp : public register_based {
            ...
            explicit an_ind_disp(opcode_t reg) noexcept : register_based(reg) {}

            template<typename T>
            inline T read(execution_context& context) {
                address_t address = this->calc_address(context._cpu);
                return context._memory.read<T>(address);
            }
        ...
        private:
            inline address_t calc_address(cpu& c) {
                address_t address = c.a(_register). template read<address_t>();
                address_t displacement = sign_extend(c.pc().fetch_extension_word());
        ...
        struct an_ind_idx : public register_based {
            ...
            an_ind_idx(opcode_t reg) : register_based(reg) {}

            template<typename T>
            inline T read(execution_context& context) {
                address_t address = calc_extension_word_address<brief_extension_word>(context._cpu.a(_register).l(),
                                                                                      context);
                return context._memory. read<T>(address);
            }
        ...
        struct abs_short {
            ...
            inline T read(execution_context& context) {
                address_t address = this->calc_address(context._cpu);
                return context._memory.template read<T>(address);
            }

            template<typename T>
            inline void write(execution_context& context, T value) {
                address_t address = this->calc_address(context._cpu);
                context._memory. template write(address, value);
            }

        private:
            inline address_t calc_address(cpu& c) { return sign_extend<word_t>(c.fetch_extension_word());}
        };
        ...
        struct abs_long {
            ...
            inline void write(execution_context& context, T value) {
                address_t address = this->calc_address(context._cpu);
                context._memory. template write<T>(address, value);
            }
        private:
            inline address_t calc_address(cpu& c) {
                return
        ...
        struct pc_ind_disp {
            ...
            template<typename T>
            inline T read(execution_context& context) {
                address_t address = context._cpu.pc().get_pc();
                address += sign_extend<word_t>(context._cpu.fetch_extension_word());
                return context._memory.template read<T>(address);
            }
        };
        ...
        struct pc_ind_idx {
            ...
            inline T read(execution_context& context) {
                ...
                address_t address = calc_extension_word_address<brief_extension_word>(context._cpu.pc().get_pc(),
                                                                                      context);
                return context._memory.template read<T>(address);
            }
        };
        ...
        struct imm {
            ...
            inline T read(execution_context& context) {
                ...
                if constexpr(is_long<T>) {
                    return
                        (static_cast<long_t>(c.fetch_extension_word()) << 16) |
                        static_cast<long_t>(c.fetch_extension_word());
                } else if constexpr(is_word<T>) {
                    return c.fetch_extension_word();
                } else if constexpr(is_byte<T>){
                    return static_cast<T>(c.fetch_extension_word());
                }
            }
        };
        ...
        concept ea_mode =
            ...
            requires (mode m, execution_context& context) {
            {m.template read<byte_t>(context)} -> std::same_as<byte_t>;
            {m.template read<word_t>(context)} -> std::same_as<word_t>;
            {m.template read<long_t>(context)} -> std::same_as<long_t>;
            };
        ...
        static inline T read(mode m, execution_context& context) {
            static_assert(ea_mode<mode>);
            return m. template read<T>(context);
        }

        template<alterable mode, integer T>
        static inline void write(mode m, execution_context& context, T value) {
            m.template write<T>(context, value);
        }
